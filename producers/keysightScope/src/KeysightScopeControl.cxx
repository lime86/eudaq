...
int main(int /*argc*/, char **argv) {
  ...
  eudaq::Option<int> hsmode(op, "hm", "handshakemode", 0x3F, "nohandshake",
                            "Bit mask for handshake mode. Set to zero and in "
                            "this mode the TLU issues a fixed-length pulse on "
                            "the trigger line (0 = no hand shake)");
  ...
  eudaq::Option<std::vector<std::string>> ipsel(
      op, "i", "dutinputs", "value", ",",
      "Selects the DUT inputs (values: RJ45,LEMO,HDMI,NONE)");
  eudaq::Option<int> emode(
      op, "e", "error-handler", 2, "value",
  ...
  eudaq::Option<int> strobelength(
      op, "l", "strobelength", 100, "cycles",
      "Length of 'on' time for timing strobe in clock cycles");
  eudaq::Option<int> enabledutveto(
      op, "b", "dutveto", 0, "mask",
      "Mask for enabling veto of triggers ('backpressure') by rasing DUT_CLK");
  eudaq::Option<int> lvpowervctrl(op, "pw", "powervctrl",
                                  (int)PMT_VCNTL_DEFAULT, "mV",
                                  "PMT Vcntl, range from 0 to 1000 [or 2000]");
  eudaq::Option<int> pmtvcntl(
      op, "pv", "pmtvcntl", -1, "mV",
      "PMT Vcntl (will override \"pw\" value if set to 0 or greater)");
  eudaq::Option<int> pmtvcntl_1(
      op, "p1", "pmtvcntl1", -1, "mV",
      "PMT 1 Vcntl (will override \"pv\" value if set to 0 or greater)");
  eudaq::Option<int> pmtvcntl_2(
      op, "p2", "pmtvcntl2", -1, "mV",
      "PMT 2 Vcntl (will override \"pv\" value if set to 0 or greater)");
  eudaq::Option<int> pmtvcntl_3(
      op, "p3", "pmtvcntl3", -1, "mV",
      "PMT 3 Vcntl (will override \"pv\" value if set to 0 or greater)");
  eudaq::Option<int> pmtvcntl_4(
      op, "p4", "pmtvcntl4", -1, "mV",
      "PMT 4 Vcntl (will override \"pv\" value if set to 0 or greater)");
  eudaq::Option<int> pmtvcntlmod(
      op, "pm", "pmtvcntlmod", 0, "value",
      "0: unmodified (<= 1000mV); 1: modified Vcntl range 0 to 2000mV");

  eudaq::OptionFlag nots(op, "n", "notimestamp",
                         "Do not read out timestamp buffer");
  eudaq::OptionFlag quit(op, "q", "quit", "Quit after configuring TLU");
  eudaq::OptionFlag pause(op, "u", "wait-for-user",
                          "Wait for user input before starting triggers");
  eudaq::Option<std::string> sname(
      op, "s", "save-file", "", "filename",
      "The filename to save trigger numbers and timestamps");
  eudaq::Option<std::string> trace(
      op, "z", "trace-file", "", "filename",
  ...
  try {
    ...
    for (int i = 0; i < TLU_PMTS; i++) {
      pmtvcntl_values[i] =
          pmtvcntl.Value() == -1 ? lvpowervctrl.Value() : pmtvcntl.Value();
    }

    if (pmtvcntl_1.Value() != -1) {
      pmtvcntl_values[0] = pmtvcntl_1.Value();
      pmt_override_set = 1;
    }
    if (pmtvcntl_2.Value() != -1) {
      pmtvcntl_values[1] = pmtvcntl_2.Value();
      pmt_override_set = 1;
    }
    if (pmtvcntl_3.Value() != -1) {
      pmtvcntl_values[2] = pmtvcntl_3.Value();
      pmt_override_set = 1;
    }
    if (pmtvcntl_4.Value() != -1) {
      pmtvcntl_values[3] = pmtvcntl_4.Value();
      pmt_override_set = 1;
    }
    ...
    std::cout << "Using options:\n"
              ...
              << (trigg.Value() > 0
                      ? " ms (" + to_string(1e3 / trigg.Value()) + " Hz)"
                      : std::string()) << "\n"
              ...
              << "\n"
              << "PMT Vcntl Mod = " << pmtvcntlmod.Value()
              << " (0: standard [<= 1000mV], else: modified for 2000mV range)"
              << std::endl
    ...
    if (pmt_override_set) {
      ...
      TLU.SetPMTVcntl(
          pmtvcntl_values[0]); // Since all 4 are the same (saves I/O to TLU)
    }
    ...
    std::cout << "TLU Version = " << TLU.GetVersion() << "\n"
              << "TLU Serial number = "
              << eudaq::hexdec(TLU.GetSerialNumber(), 4) << "\n"
